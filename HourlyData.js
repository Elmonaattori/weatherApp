...
export default class HourlyData extends Component {
    ...
    static navigationOptions = {
        tabBarLabel: 'Tuntiennuste',
        tabBarIcon: () => (
            <Image source= {require("./tabBarIcons/kello.png")} style ={{height: 30, width: 30}}/>
        )
    }
    ...
    FetchData() {
        ...
        .then( (responseJson) => {
            this.setState({
                isLoading: false,
                weatherData: responseJson.hourly.data,
            })
        })
        .catch((error) => {
            console.log(error)
    ...
    render () {
        ...
        if (this.state.isLoading) {
            ...
            return(
                <View style = {{backgroundColor: 'rgb(190, 80, 50)'}}>
                    <FadeView>
                        <ScrollView style={{flexGrow: 6}}> 
                        {
                            this.state.weatherData.map((day, i) => {
                                ...
                                var time = a.getHours() + ':' + a.getMinutes() + '0'
                            
                                if (day.icon === 'rain' || day.icon === 'cloudy' || day.icon === 'fog' || day.icon === 'cloudy') {
                                    return (
                                        <View key={i} style={styles.cloudydayitem}>                      
                                            <View style={styles.timeContainer}>
                                                ...
                                                <Text style = {styles.smallTextStyle}>{time}</Text>                             
                                            </View>                  
                                            <View style={{flexDirection: 'row'}}>
                                                <SmallWeatherView>{day.icon}</SmallWeatherView>
                                                <View style = {styles.infoContainer}>
                                                    <Text style={styles.textStyle}>
                                                        {((day.temperature - 32)/1.8).toFixed(0) + ' °C'}
                                                    </Text>
                                    ...
                                    return(
                                        <View key={i} style={styles.nightitem}>                           
                                            <View style={styles.timeContainer}>
                                                <Text style={styles.textStyle}>
                                                    {this.state.weekdays[weekday]}
                                                </Text>
